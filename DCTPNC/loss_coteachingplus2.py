...
def loss_coteaching_plus(logits, logits2, labels, forget_rate, truth_label,y_3):
    outputs = F.softmax(logits, dim=1)
    outputs2 = F.softmax(logits2, dim=1)
    outputs3=F.softmax(y_3, dim=1)
    ...
    if len(disagree_id) > 0:
        ...
        loss_1, loss_2, _ = loss_coteaching(update_outputs, update_outputs2, update_labels, forget_rate,
                                            truth_label[disagree_id],update_outputs3)
    else:
        loss_1, loss_2,num=loss_coteaching(logits, logits2, labels, forget_rate,  truth_label,y_3)
